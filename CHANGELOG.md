...
- Runtime 3.0 support is not claimed. The matching `3.0.4.2` client executable and `game_pak` are now version/hash verified, but decrypted `compact.sqlite3` and `compact.server.table.sqlite3` are not present, so server startup, login/serializer, and PlayerBots lifecycle acceptance remain open.
