...
ArcheAge PlayerBots `0.2.0-alpha.5` is suitable for source-level preview testing on the exact AAEmu bases documented below. It is not a drop-in DLL, does not include ArcheAge client/server data, and does not make the experimental 3.0 track production-ready.

## Current validation

| Track | Installer | Automated gate | Runtime status |
| --- | --- | --- | --- |
| ArcheAge 1.2 `r208022` | Installed-state check passed | 1,761 passed plus 4 intentional legacy skips | Supported host track; repeat gameplay validation is still recommended for every server |
...
- Quest commands are bounded development slices. One selected exact-NPC hunt can acquire and repeat nearby targets, and one selected static sphere objective can travel up to 100 meters, but autonomous quest selection, long-range route planning, mixed objectives, and objective chaining remain out of scope.
