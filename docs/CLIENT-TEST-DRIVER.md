...
After the transition, the runner records `/botmetrics snapshot`, focuses the exact gameplay window with a separate one-action lease, and retains bounded hash-verified capture/assertion attempts until at least two exact RGB gameplay anchors pass. A 100%-progress loading screen therefore remains a failure, not a timing-based pass. Some 3.0 client re-entry paths render the complete gameplay world before their buffered `Loading Complete` line becomes observable; in that case the fresh loading marker, named server-online transition, and exact gameplay anchors jointly establish the world-loaded result. The immutable JSON summary records every attempt and which proof path passed along with Git/profile/client identity, lifecycle snapshots, character identity, Web API truth, input audits, capture hashes/dimensions, Computer Use counts, and explicit safety declarations. A failure also writes a new retained summary and never closes or force-terminates the client.
