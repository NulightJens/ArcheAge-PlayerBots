...
/// <summary>Finds and accepts eligible NPC or doodad quests through AAEmu.</summary>
public sealed class BotQuestIntakeController
{
    ...
    /// <summary>Returns true while quest intake owns this tick.</summary>
    internal bool Step(BotRuntime runtime, BotConfig config, DateTimeOffset now)
    {
        ...
        lock (_syncRoot)
        {
            ...
            // Nearby quests outrank a remote main-story route.
    ...
    /// <summary>Releases intake-owned movement when active work takes priority.</summary>
    ...
    private bool StepPlan(BotRuntime runtime, BotConfig config, DateTimeOffset now)
    {
        ...
        if (distance > config.QuestIntakeInteractionRadius)
        {
            ...
            // Keep the accepted approach point stable while moving.
    ...
    private static BaseUnit ResolveGiver(
        ...
        kind switch
        {
            ...
            BotQuestGiverKind.Doodad => ResolveDoodad(bot, objectId),
            _ => null
        };
        ...
        uint questId)
    {
        ...
        return kind switch
        {
            ...
            BotQuestGiverKind.Doodad => ValidateDoodadQuest(bot, objectId, questId),
            _ => false
        };
    }

    private static Doodad ResolveDoodad(Character bot, uint objectId)
    {
#if PLAYERBOTS_AAEMU_3_0
        return null;
#else
        return bot?.ParentWorld?.GetDoodad(objectId);
#endif
    }

    private static bool ValidateDoodadQuest(Character bot, uint objectId, uint questId)
    {
#if PLAYERBOTS_AAEMU_3_0
        return false;
#else
        return ResolveDoodad(bot, objectId) is { } doodad &&
