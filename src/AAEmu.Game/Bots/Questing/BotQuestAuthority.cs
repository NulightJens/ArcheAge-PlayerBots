...
internal sealed class BotQuestAuthority : IBotQuestAuthority
{
    ...
    internal static (BotQuestObjectiveShape Shape, BotQuestMonsterHuntObjective? Objective, string Reason)
        InterpretObjective(Quest quest)
    {
        if (quest == null || !quest.QuestSteps.TryGetValue(QuestComponentKind.Progress, out var progress))
            return (BotQuestObjectiveShape.Unsupported, null, "no_progress_step");
        ...
        if (objectiveActs.Length == 0)
            return (BotQuestObjectiveShape.Unsupported, null, "no_active_objective");
        if (objectiveActs.Length != 1)
            return (BotQuestObjectiveShape.Ambiguous, null, "multiple_active_objectives");

        var act = objectiveActs[0];
        if (act.Template is not QuestActObjMonsterHunt monster)
            return (BotQuestObjectiveShape.Unsupported, null, $"unsupported_{act.Template.GetType().Name}");
        ...
        {
            return (BotQuestObjectiveShape.Invalid, null, "invalid_monster_hunt_objective");
        }
