...
/// </summary>
public sealed partial class BotQuestCommand : ICommand
{
    ...
    public string GetCommandLineHelp() =>
        ...
        "party create <leader-id> <second-id> <third-id>, party status, " +
        "party support <leader-id>, party independent <member-id>, " +
        "interact <botId> <inspect|approach|use> <doodadObjId> [skillId], " +
#endif
    ...
    {
#if !PLAYERBOTS_AAEMU_3_0
        if (args is { Length: > 0 } && args[0].Equals("party", StringComparison.OrdinalIgnoreCase))
        {
            new BotQuestPartyHandler(this).Execute(character, args[1..], messageOutput);
            return;
        }
        if (args is { Length: > 0 } && args[0].Equals("interact", StringComparison.OrdinalIgnoreCase))
        {
            Interact(args[1..], messageOutput);
            return;
        }
#endif
